...
class SoniTranslate(SoniTrCache):
    ...
    def multilingual_media_conversion(
        ...
        if not YOUR_HF_TOKEN:
            ...
            # Specify the path to your .env if it’s not in the working directory:
            # load_dotenv(dotenv_path="/path/to/.env")
            load_dotenv()  # reads from .env in cwd by default
        ...
        if not self.task_in_cache("subs_and_edit", [
            ...
            if output_format_subtitle == "ass":
                convert_ori = "ffmpeg -hwaccel cuda -i sub_ori.srt sub_ori.ass -y"
                convert_tra = "ffmpeg -hwaccel cuda -i sub_tra.srt sub_tra.ass -y"
        ...
        if not self.task_in_cache("mix_aud", [
            ...
            command_volume_mix = f'ffmpeg -hwaccel cuda -y -i {base_audio_wav} -i {dub_audio_file} -filter_complex "[0:0]volume={volume_original_audio}[a];[1:0]volume={volume_translated_audio}[b];[a][b]amix=inputs=2:duration=longest" -c:a libmp3lame {mix_audio_file}'
            command_background_mix = f'ffmpeg -hwaccel cuda -i {base_audio_wav} -i {dub_audio_file} -filter_complex "[1:a]asplit=2[sc][mix];[0:a][sc]sidechaincompress=threshold=0.003:ratio=20[bg]; [bg][mix]amerge[final]" -map [final] {mix_audio_file}'
        ...
        if burn_subtitles_to_video:
            ...
            if self.burn_subs_id != hashvideo_text:
                try:
                    ...
                    command = f"ffmpeg -hwaccel cuda -i {base_video_file} -y -vf subtitles=sub_tra.srt -max_muxing_queue_size 9999 {vid_subs}"
                    run_command(command)
        ...
        if not self.task_in_cache("output", [
            ...
            run_command(
                f"ffmpeg -hwaccel cuda -i {base_video_file} -i {mix_audio_file} -c:v copy -c:a copy -map 0:v -map 1:a -shortest {video_output_file}"
            )
